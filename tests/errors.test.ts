...
describe("handleCopilotError", () => {
    ...
    it("throws LoadAPIKeyError for auth-like messages", () => {
        expect(() =>
            handleCopilotError(new Error("not authenticated"))
        ).toThrow(LoadAPIKeyError);
    });

    it("throws LoadAPIKeyError for token expired", () => {
        expect(() =>
            handleCopilotError(new Error("token expired"))
        ).toThrow(LoadAPIKeyError);
    });

    it("throws LoadAPIKeyError for please login", () => {
        expect(() =>
            handleCopilotError(new Error("please login first"))
        ).toThrow(LoadAPIKeyError);
    });

    it("throws APICallError for other errors", () => {
        expect(() =>
            handleCopilotError(new Error("Something went wrong"))
