...
import { convertToCopilotMessages } from "../src/convert-to-copilot-messages.js";
...
describe("convertToCopilotMessages", () => {
    ...
    it("converts tool results with text output", () => {
        ...
        expect(result.prompt).toContain(
            "Tool result (get_weather): Sunny, 72°F"
        );
    });
    ...
    it("converts tool results with json output", () => {
        ...
        const result = convertToCopilotMessages(prompt);
        expect(result.prompt).toContain(
            'Tool result (search): {"results":["a","b"]}'
        );
    });
    ...
    it("converts tool results with error-text output", () => {
        ...
        const result = convertToCopilotMessages(prompt);
        expect(result.prompt).toContain(
            "Tool result (fetch): Error: Network failed"
        );
    });
    ...
    it("converts tool results with execution-denied output", () => {
        ...
        const result = convertToCopilotMessages(prompt);
        expect(result.prompt).toContain(
            "Tool result (delete_file): [Execution denied: Not allowed]"
        );
    });
        ...
        const prompt: LanguageModelV3Prompt = [
            {
                role: "user",
                content: [
                    { type: "file", data: "http://example.com/file.png" },
                ],
            },
        ];
        const result = convertToCopilotMessages(prompt);
